...
std::pair<int, db::DataType *> IndexBlock::get_index_ralationInfo()
{
    RelationInfo *info = table_->info_;
    unsigned int key = info->key;
    DataType *type = info->fields[key].type;
...
unsigned short IndexBlock::searchRecord(void *buf, size_t len)
{
    ...
    auto index_info = get_index_ralationInfo();
    // 获取key位置
    unsigned int key = index_info.first;
    DataType *type = index_info.second;

    // 调用数据类型的搜索
    return type->search(buffer_, key, buf, len);
}
...
IndexBlock::splitPosition(size_t space, unsigned short index)
{
    ...
    std::pair<int, db::DataType *> index_info = get_index_ralationInfo();
    // 获取key位置
    unsigned int key = index_info.first;
